...
for d in $dirs; do
   ...
   for p in $pus; do
      save_plots.py reemulated=y reweighed=y unweighed=n unemulated=n plot_only="$d\$" "cmp/{d}/reco${p}_{p}.eps" \
         $(ls plots_{data,mc}_*reco-${p}*.root|grep -v no|grep -v Cre) &
      if [ $p == "2012C" ]; then
         save_plots.py reemulated=y reweighed=y unweighed=n unemulated=n plot_only="$d\$" "cmp/{d}/reco${p}noE_{p}.eps" \
            $(ls -U plots_data_*reco-${p}noE*.root plots_mc_*reco-${p}{,ext2,ext3}noE*.root) &
         save_plots.py reemulated=y reweighed=y unweighed=n unemulated=n plot_only="$d\$" "cmp/{d}/reco${p}noH_{p}.eps" \
